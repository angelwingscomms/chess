...
<div class="fixed inset-0 grid place-items-center bg-canvas p-4">
	<div class="max-w-sm text-center space-y-4">
		{#if data.success}
			...
			<p class="text-sm text-muted">{data.tokens.toLocaleString()} tokens have been added to your account.</p>
			<p class="text-xs text-muted/60">New balance: {data.balance.toLocaleString()} tokens</p>
		{:else}
