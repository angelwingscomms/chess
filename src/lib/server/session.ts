...
export async function encode_session(data: { id: string; name: string; picture?: string }): Promise<string> {
  const p = { u: data.id, n: data.name, p: data.picture, e: Date.now() + 604800000 };
...
export async function decode_session(c: string | undefined | null): Promise<{ user: { id: string; name: string; picture?: string } } | null> {
  if (!c) return null;
  ...
  try {
    ...
    return { user: { id: p.u, name: p.n, picture: p.p } };
  } catch { return null; }
}
