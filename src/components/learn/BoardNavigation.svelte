...
<button title="Next board" class="grid size-8 place-items-center rounded-full bg-canvas text-ink transition-colors hover:text-primary disabled:text-muted" onclick={() => s.go_forward_board()} disabled={board_history_idx >= board_history.length - 1}>
	<ArrowRightIcon size={15} strokeWidth={1.8} />
</button>
<button title="Flip board" class="grid size-8 place-items-center rounded-full bg-canvas text-ink transition-colors hover:text-primary" onclick={() => s.chessRef?.toggleOrientation()}>
	<span style="display:inline-flex;transform:scaleX(-1)"><FlipIcon size={15} strokeWidth={1.8} /></span>
</button>
<button title="Switch sides" class="grid size-8 place-items-center rounded-full bg-canvas text-ink transition-colors hover:text-primary" onclick={() => s.flipColor()}>
